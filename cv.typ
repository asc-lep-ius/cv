...
== Professional Summary
  ...
  3.5 years at Finmatics (Visma Group), building LLM-integrated microservices with Google Gemini and multi-agent development tooling — a Conductor orchestrating 8 subagents with parallel execution and GitLab audit trail. Personal projects span RAG pipelines (Athena), novel LLM architectures (TAME-Swarm), and a full-stack student toolkit (Sophia — 35k lines, 1,530+ tests). Available from June 2026.
] else if compact [
...
#experience(
  ...
  - Built *Hephaestus*, a self-hosted homelab: 9+ Docker Compose services (GitLab CE, GPU ML platform, Ollama, CalDAV), Caddy reverse proxy with TLS, Tailscale mesh ACLs, Kaniko CI builds, automated backups
  ]
] else [
  ...
  - *Hephaestus* — self-hosted homelab: 9+ Docker services (GitLab CE, GPU ML, Ollama), Caddy TLS, Tailscale mesh
] else if not ai [
...
== Technical Skills
  ...
  #project(
    ...
    description: "Novel LLM architecture grounded in Michael Levin's TAME framework. VCG auction-based expert routing replacing standard MoE gating, with activation steering for drift-resistant alignment. Prototype on Gemma-2B/Mistral-7B. 3,000+ lines.",
  )
  ...
  #project(
    ...
    description: "RAG pipeline transforming books/PDFs into knowledge graphs. 5-layer architecture (Parse → Graph → Analyze → Derive → Review) with Pydantic-validated LLM structured output. 7,000+ lines.",
  )

  #project(
    name: "Sophia (σοφία)",
    tech: "Python · ChromaDB · faster-whisper · Gemini/Groq/Ollama · Sentence Transformers · aiosqlite",
    description: "Constructivist student toolkit for TU Wien. Hexagonal architecture, GPU transcription, vector search, multi-provider LLM integration, FSRS spaced repetition. 35,000+ lines, 1,530+ tests.",
  )
] else if not compact [
  == Key Projects
  ...
  #project(
    ...
    description: "Self-hosted homelab running 9+ containerized services: GitLab CE, GPU-accelerated ML platform, Ollama LLM inference, CalDAV, media server. Caddy reverse proxy with internal TLS, Tailscale mesh networking with strict ACLs, dual-GPU CI/CD runners (Kaniko), automated backups with disaster recovery procedures.",
  )
