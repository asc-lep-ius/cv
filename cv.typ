...
- Built REST APIs for prediction phrases, bank statement Excel exports, and HubSpot CRM integrations with Django REST Framework
  ...
  - Built an *auto-onboarding system* with HubSpot CRM integration: company provisioning, user management, and branding config
...
== Technical Skills
  ...
  #project(
    ...
    description: "Novel LLM architecture grounded in Michael Levin's TAME framework. Replaces standard MoE routing with a VCG second-price auction where experts compete with wealth-based bids for emergent specialization. Activation steering vectors provide drift-resistant alignment via homeostatic control. Working prototype on Gemma-2B/Mistral-7B with live Gradio visualization. 3,000+ lines.",
  )

  #project(
    name: "Finmatics MCP Server",
    tech: "FastMCP · httpx · Pydantic · tenacity · Cursor · Claude Code · VS Code Copilot",
    description: "Custom MCP server for running API and smoke tests against staging/local via natural language. Shared HTTP client, JWT auto-refresh, production safety blocklist, exponential-backoff retry. Works in VS Code Copilot, Claude Code, and Cursor.",
  )

  #project(
    name: "Auto-Onboarding System",
    tech: "Python · HubSpot Workflow Custom Code Blocks",
    description: "End-to-end customer provisioning automation. CRM integration for company/user creation, branding config, and role assignment. Still saving hours of repetitive work after years of operation without maintenance.",
  )
]
