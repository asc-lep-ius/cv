...
#let dense = sys.inputs.at("dense", default: "false") == "true"
...
#let cv(
  ...
  set page(
    ...
    margin: if compact {
      ...
      (top: 1.25cm, bottom: 1.0cm, left: 1.5cm, right: 1.5cm)
    } else {
  ...
  set text(font: "Inter", size: if compact { 8.5pt } else if dense { 9.2pt } else { 9.5pt }, fill: luma(30))
  set par(justify: true, leading: if compact { 0.5em } else if dense { 0.54em } else { 0.6em })
  ...
  show heading.where(level: 2): it => {
    v(if compact { 0.25em } else if dense { 0.3em } else { 0.45em })
    block(width: 100%, {
      text(size: if compact { 10pt } else if dense { 10.6pt } else { 11pt }, weight: "bold", fill: accent-color, upper(it.body))
      v(-0.35em)
      line(length: 100%, stroke: 0.5pt + accent-color)
    })
    v(if compact { 0.1em } else if dense { 0.12em } else { 0.2em })
  }
  ...
  set list(indent: 0.3em, body-indent: 0.4em, spacing: if compact { 0.35em } else if dense { 0.42em } else { 0.55em }, marker: text(fill: accent-color, "▸"))
